...
Using names() to change the name of the column "Homicide rate in Europe over long-term (per 100,000) (homicides per 100,000 people)" to "homicides_per_100k".
...
Western_Europe %>% 
  ...
  labs(title = "Homocide rate in Western Europe, 1300-2016", x="Year",y="Homicides (per. 100k of population")
...
# Adding the facet_wrap() function
...
```{r legend_fixed, eval = TRUE}
# Adding the facet_wrap() function

Western_Europe %>% 
  ggplot(aes(x=Year,y=homicides_per_100k))+
  geom_smooth(aes(fill = Entity))+
  labs(title = "Homicide rate in Western Europe, 1300-2016", x="Year",y="Homicides (per. 100k of population")+
  facet_wrap(~Entity,nrow=3,ncol=2)+
...
Moving the legend from the current position on the side to below the facet, and label it "Country" instead of "Entity"
...
Firstly, I load the data and assign it as object "kongerækken"
```{r loading data}
kongerækken <- read_csv2("data/kongerækken.csv")
```



Therearfter I create the coloumns for plotting:
```{r danish_monarchs_duration_of_rule, eval = TRUE}
...
And lastly use ggplot() to create a visualisation 
```{r visualisation_kings}
...
kongerækken_plotting %>%
  ...
  labs(title="Danish monarch's duration of rule, 900-2023", x = "Middle of monarchs' rule",
...
The chart shows danish monarch's duration of rule in the period, year 900-2023. Monarch's year of death on the x-axis, in order to sort them by period. In order to include Dronning Margrethe 2.'s duration of rule, I've put 2023 as the year of death and end of reign. Gorm den Gamle's start of reign is unclear, and therefore left out. Interregnums left out: 1332 - 1340 and 1533 - 1534

Compared to the homicides above, the trend of danish monarch's duration of rule increases over time, while the homicides of Western Europe decreases over time. In addition, the trend of homicides are more steep
...
Of course, it depends on how you were to define “civilised”. However, in terms of it being a question about a society having a well-developed, stable, and fair justice system and governmental rule and people treating each other fairly, there are some points to be made regarding the visualisations of “Homicide rate in Western Europe, 1300-2016” and “Danish monarch's duration of rule, 900-2023”. It can be argued that the descending trend of homicides in Western Europe in the period 1300-2016 is an expression of increasing amount of civility in society.  A civilised society would be expected to bring down the frequency of homicides by enforcing a justice system. In “Danish monarch’s duration of rule in the period 900-2023” the overall trend of increasing duration of rule of Danish monarchs can also be seen as a rise of civility, because it can be an expression of a stable government rule and few to no regicides. 
